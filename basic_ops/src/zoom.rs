...
fn zoom_out_pixel(view: SubImage<&DynamicImage>, factors: (u8, u8)) -> Rgba<u8> {
    ...
    let mut out = Rgba::<u8>([Default::default(); 4]);
...
pub fn saturate_dimension(dimension: u32, factor: u8, boundary: u32) -> (u32, u32) {
...
pub fn zoom_out(img: DynamicImage, factors: (u8, u8)) -> DynamicImage {
    ...
    for x in 0..new_img.width() {
        let (next_x, factor_x) = saturate_dimension(x, factors.0, img.width());

        for y in 0..new_img.height() {
            let (next_y, factor_y) = saturate_dimension(y, factors.1, img.height());
